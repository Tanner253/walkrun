 ...
 */
export async function logSolPayout(walletAddress, amount, txSignature, status = 'pending', type = null) {
  const db = await getDatabase();
  ...
  if (type) payoutDoc.type = type;
  if (status === 'completed') payoutDoc.completedAt = new Date();
  
  await payouts.insertOne(payoutDoc);
}
